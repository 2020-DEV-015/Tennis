...
class TennisTests: XCTestCase {
    ...
    func testPlayerAName() {
        ...
        XCTAssertEqual(expectedName, tennis.getPlayerName(for: .playerA))
    }
    
    func testPlayerBName() {
        let expectedName = "Player B"
        XCTAssertEqual(expectedName, tennis.getPlayerName(for: .playerB))
    }
    ...
    private func increaseGame(pointBy: Int, forPlayer player: PlayerSide) {
        let range = pointBy
        for _ in 1...range {
            tennis.scored(byPlayer: player)
        }
    }
}
